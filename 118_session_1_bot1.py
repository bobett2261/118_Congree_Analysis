...
def parse_xml_data(xml_data):
    ...
    data = {
        'Majority': metadata.findtext("majority", default='N/A') if metadata is not None else 'N/A',
        'Congress': metadata.findtext("congress", default='N/A') if metadata is not None else 'N/A',
        'Session': metadata.findtext("session", default='N/A') if metadata is not None else 'N/A',
        'Chamber': metadata.findtext("chamber", default='N/A') if metadata is not None else 'N/A',
        'Roll Call Number': metadata.findtext("rollcall-num", default='N/A') if metadata is not None else 'N/A',
        'Legislation Number': metadata.findtext("legis-num", default='N/A') if metadata is not None else 'N/A',
        'Vote Question': metadata.findtext("vote-question", default='N/A') if metadata is not None else 'N/A',
        'Vote Type': metadata.findtext("vote-type", default='N/A') if metadata is not None else 'N/A',
        'Vote Result': metadata.findtext("vote-result", default='N/A') if metadata is not None else 'N/A',
        'Action Date': metadata.findtext("action-date", default='N/A') if metadata is not None else 'N/A',
        'Action Time': metadata.find(".//action-time").attrib.get("time-etz", 'N/A') if metadata is not None else 'N/A',
        'Description': metadata.findtext("vote-desc", default='N/A') if metadata is not None else 'N/A',
...
def main(start_roll_call, end_roll_call, file_path, year=2023):
...
if __name__ == "__main__":
    file_path = '/Users/spencer/Desktop/Roll Call/Actual Data/Excel Files/118_Session_1_All_Data.xlsx'  # Update with your actual path
    start_roll_call = 151  # Use the actual starting roll call number you wish to process
    end_roll_call = 250  # Use the actual ending roll call number you wish to process
